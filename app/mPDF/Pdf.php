...
/**
 * Custom package from: https://github.com/niklasravnsborg/laravel-pdf
 */
class Pdf
{
    protected $config = [];
    ...
    public function __construct($html = '', $config = [])
    {
        ...
        $mpdf_config = [
            ...
            'tempDir' => storage_path() . '/temp', // temporary directory
        ...
        if (
            isset($this->config['instanceConfigurator']) &&
            is_callable($this->config['instanceConfigurator'])
        ) {
    ...
    protected function addCustomFontsConfig($mpdf_config)
    {
        ...
        // $fontDirs = $this->mpdf->getDefaults()['fontDir'];
        // dd($fontDirs);
        ...
        $mpdf_config['fontDir'] = array_merge($fontDirs, [
            Config::get('pdf.font_path'),
        ]);
        $mpdf_config['fontdata'] = array_merge(
            $fontData,
            Config::get('pdf.font_data')
        );
